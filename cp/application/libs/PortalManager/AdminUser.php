...
class AdminUser
{
	...
	function getStats(){
		$ret = array();
			...
			ORDER BY
				u.elkuldve DESC LIMIT 0, 10;")->fetchAll(\PDO::FETCH_ASSOC);
			$ret[lastMessages][data] = $lastMsg;
		return $ret;
	}

	function getSlideShow(){
		$q = "SELECT * FROM slideshow ORDER BY lathato DESC, sorrend ASC";
	...
	function getMegrendelesek($arg = array()){
		...
		WHERE o.ID IS NOT NULL";
			...
			if($arg[filters][nameemail]){
				$q .= " and (o.nev like '%".trim($arg[filters][nameemail])."%' or o.email like '%".trim($arg[filters][nameemail])."%')"	;
			}
		...
		$q .= "
		ORDER BY oa.sorrend ASC, o.idopont DESC";
		...
		$cont 	= 0;
		foreach($data as $d){
			$arg['kedvezmeny'] = $d[kedvezmeny_szazalek];
	...
	public function getOrderItems($orderID, $arg = array()){
		...
		foreach($data as $d){
			if( $arg[kedvezmeny] > 0 ) {
				...
				\PortalManager\Formater::discountPrice( $d[subAr], $arg[kedvezmeny] );
				\PortalManager\Formater::discountPrice( $d[egysegAr], $arg[kedvezmeny] );
			}
			$totalPrice += $d[subAr];
			$tetel 		+= $d[me];
			$bdata[] = $d;
	...
	function addSlideShow($post){
		...
		$this->db->insert('slideshow',
			array_combine(
			array('url','sorrend','kep','lathato', 'focim', 'alcim', 'focim_link', 'focim_link_text'),
			array($url,$sorrend,$img,$lathato,$focim, $alcim, $focim_link, $focim_link_text)
			)
		);
	}
	...
	function copyTermek($termekID = 0, $copyNum = 0){
		...
		for($c = 1; $c <= $copyNum; $c++){
			...
			// Termék másolás
				...
				// Update
				$this->db->query("UPDATE copyTermek SET ID = null, letrehozva = now(), lathato = 0;");
				// Insert
	...
	*/
	...
	function getSearchMostStat(){
		$date = date('Y-m');
		$q .= "SELECT *, sum(me) as me FROM `stat_kereses` WHERE szoveg != '' and datum LIKE '$date%' GROUP BY szoveg ORDER BY sum(me) desc LIMIT 0,5";
	...
	*******************************/
	...
	public function listTermekImages( $tid )
	{
		$images = array();

		$q = "SELECT ID, kep FROM shop_termek_kepek WHERE termekID = $tid";
		extract($this->db->q($q,array('multi'=>'1')));

		foreach($data as $d){
			$kep = PRODUCTSIMGDIR.str_replace('src/products/all/','all/',$d['kep']);

			$image = array(
				'src' => $kep,
				'view' => SOURCE . str_replace('src/products/all/','products/all/', $d[kep]),
				'exists' => file_exists($kep)
			);
			$images[] = $image;

			// thb 75
			$kep = PRODUCTSIMGDIR.str_replace('src/products/all/','all/thb75_',$d['kep']);
			$image = array(
				'src' => $kep,
				'view' => SOURCE . str_replace('src/products/all/','products/all/thb75_', $d[kep]),
				'exists' => file_exists($kep)
			);
			$images[] = $image;

			// thb 150
			$kep = PRODUCTSIMGDIR.str_replace('src/products/all/','all/thb150_',$d['kep']);
			$image = array(
				'src' => $kep,
				'view' => SOURCE . str_replace('src/products/all/','products/all/thb150_', $d[kep]),
				'exists' => file_exists($kep)
			);
			$images[] = $image;
		}
		...
		if (count($images) == 0) {
			$profil = $this->db->query("SELECT profil_kep FROM shop_termekek WHERE ID = {$tid};")->fetchColumn();

			if (!empty($profil)) {
				$kep = PRODUCTSIMGDIR.str_replace('src/products/all/','all/',$profil);

				$image = array(
					'src' => $kep,
					'view' => SOURCE . str_replace('src/products/all/','products/all/', $profil),
					'exists' => file_exists($kep)
				);
				$images[] = $image;
			}
	...
	public function clearTermekImages($post)
	{
		extract($post);
		...
		// Db delete
		$this->db->query("DELETE FROM shop_termek_kepek WHERE termekID = {$id};");
