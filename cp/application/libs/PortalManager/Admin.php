...
class Admin
{
	...
	public function checkImportProducts( $xml_list = null )
	{
		...
		if ( $xml_list ) {
			...
			foreach ( $xml_list as $item ) {
				...
				$in['data']['b2b_netto_ar']	= $in['source']['b2bprice'];
				$in['data']['b2b_brutto_ar']	= $in['source']['b2bprice'] * 1.27;
				...
				**/
				...
				$check = $this->db->query( sprintf("
					SELECT 				ID, netto_ar, brutto_ar, b2b_netto_ar, b2b_brutto_ar, b2b, rovid_leiras, nev, rovid_leiras, szin_kod, akcios, akcios_netto_ar, akcios_brutto_ar, szin, meret, raktar_supplierid, link_hashkeys, kategoria_hashkeys, csoport_kategoria
					FROM 				shop_termekek
				...
				if ( $in['status']['exists'] == 1 ) {
					...
					// B2B
					if ( $in['data']['b2b'] != $exist_data['b2b'] ) {
						$update_rows[] = 'b2b';
						$want_update = 1;
					}

					if ( $in['data']['b2b_netto_ar'] != $exist_data['b2b_netto_ar'] ) {
						$update_rows[] = 'b2b_netto_ar';
						$update_rows[] = 'b2b_brutto_ar';
						$want_update = 1;
					}
					// E:B2B
	...
	**/
	...
	{
		if(empty($cat_id) || is_null($cat_id)) return array();
		...
		while( $walk ) {
			$iq = "SELECT ID, szulo_id, neve, hashkey FROM shop_termek_kategoriak WHERE ID = $cat_id;";
			//echo $iq;
			$q = $this->db->query( $iq );
	...
	private $logorderstep = 0;
	public function logOrderChange( $order, $line, $comment, $allapot  )
	{
		return false;
		...
		$this->db->insert(
			...
			array(
				'orderhash' => $order,
				'allapot' 	=> $allapot,
				'step' 		=> $this->logorderstep,
				'line' 		=> $line,
				'comment' 	=> $comment,
				'microtimeon' => microtime()
			)
		);
		$this->logorderstep++;
	}
	...
	function saveOrderData($orderID, $post){
		...
		$this->logOrderChange( $orderID, __LINE__, 'START', $post[allapotID][$orderID] );
		$this->logOrderChange( $orderID, __LINE__, 'ID ellenőrzése', $post[allapotID][$orderID] );
		if($orderID == '') return false;
		$this->logOrderChange( $orderID, __LINE__, 'Van ID: '.$orderID, $post[allapotID][$orderID] );
		...
		$this->logOrderChange( $orderID, __LINE__, 'Megrendelés adatainak lekérése: getOrderData() - START', $post[allapotID][$orderID] );

		$orderData 	= $this->getOrderData($accessKey);

		$this->logOrderChange( $orderID, __LINE__, 'Megrendelés adatainak lekérése: getOrderData() - END - SIKERES', $post[allapotID][$orderID] );

		$this->logOrderChange( $orderID, __LINE__, 'Felhasználó adatok lekérése: new User() - START', $post[allapotID][$orderID] );

		$users 		= new Users( array( 'db' => $this->db, 'admin' => true ) );

		$this->logOrderChange( $orderID, __LINE__, 'Felhasználó adatok lekérése: new User() - END - SIKERES', $post[allapotID][$orderID] );

		$this->logOrderChange( $orderID, __LINE__, 'Felhasználó adatok lekérése: new User()->get() - START', $post[allapotID][$orderID] );

		$user 		= $users->get(array( 'user' => $orderData['email'] ));

		$this->logOrderChange( $orderID, __LINE__, 'Felhasználó adatok lekérése: new User()->get() - END - SIKERES', $post[allapotID][$orderID] );

		$this->logOrderChange( $orderID, __LINE__, 'IF $user - START', $post[allapotID][$orderID] );

		if( $user ) {
			$this->logOrderChange( $orderID, __LINE__, 'IF $user - START - TRUE', $post[allapotID][$orderID] );
			$this->logOrderChange( $orderID, __LINE__, 'IF $user - START - TRUE - $totalOrderPrice QUERY', $post[allapotID][$orderID] );
			...
			$this->logOrderChange( $orderID, __LINE__, 'IF $user - START - TRUE - $totalOrderPrice QUERY: '.$totalOrderPrice, $post[allapotID][$orderID] );
		} else {
			$this->logOrderChange( $orderID, __LINE__, 'IF $user - START - FALSE', $post[allapotID][$orderID] );
			$totalOrderPrice = 0;
		}

		// Új termékek hozzáadása
		$this->logOrderChange( $orderID, __LINE__, 'Új termék hozzáadása - START', $post[allapotID][$orderID] );

		$added_new_items = array();
		if ( count($post['new_product']) > 0 )
		{
			$this->logOrderChange( $orderID, __LINE__, 'Új termék hozzáadása - START - ÚJ TERMÉKEK', $post[allapotID][$orderID] );
			...
			$this->logOrderChange( $orderID, __LINE__, 'Új termék hozzáadása - START - NINCS ÚJ TERMÉK', $post[allapotID][$orderID] );
		}
		...
		if($kedvezmeny != $kedvezmeny_pre){
			$updateData['kedvezmeny_szazalek'] 	= $kedvezmeny;
		...
		$ppp_uzlet 		= $post[pickpackpont_uzlet_kod][$orderID];
		$ppp_uzlet_pre 	= $post[prev_pickpackpont_uzlet_kod][$orderID];
		...
		$this->logOrderChange( $orderID, __LINE__, 'Megrendelés adatainak mentése', $post[allapotID][$orderID] );
		...
		if ( !empty($updateData)) {
			...
			$this->logOrderChange( $orderID, __LINE__, 'Megrendelés adatainak mentése - MENTVE', $post[allapotID][$orderID] );
		} else {
			$this->logOrderChange( $orderID, __LINE__, 'Megrendelés adatainak mentése - NINCS MENTVE - NEM VOLT VÁLTOZÁS', $post[allapotID][$orderID] );
		}
		...
		foreach($termekAllapotChange as $tac){
			$this->logOrderChange( $orderID, __LINE__, 'Termék állapot változtatás: TID'.$tac['id'].' => '.$tac['val'], $post[allapotID][$orderID] );
			$this->db->update(
				'order_termekek',
				array(
					'allapotID' => $tac[val]
			...
			$this->logOrderChange( $orderID, __LINE__, 'Termék állapot változtatás: TID'.$tac['id'].' => '.$tac['val'].' - MENTVE', $post[allapotID][$orderID] );
		}

		// Termékek törlése, amelyek törölve állapotra lett helyezve
		foreach($termek_will_delete as $dtid){
			$this->logOrderChange( $orderID, __LINE__, 'Termék törlése: '.$dtid, $post[allapotID][$orderID] );
			$this->db->query("DELETE FROM order_termekek WHERE ID = $dtid");
			$this->logOrderChange( $orderID, __LINE__, 'Termék törlése: '.$dtid.' - TÖRÖLVE', $post[allapotID][$orderID] );
		}
		...
		foreach($termekMeChange as $tac){
			$this->logOrderChange( $orderID, __LINE__, 'Termék mennyiség változtatás: TID'.$tac['id'].' => '.$tac['val'], $post[allapotID][$orderID] );
			$changedData[termekMe]	= 1;
			$this->db->update(
				'order_termekek',
				array(
			...
			$this->logOrderChange( $orderID, __LINE__, 'Termék mennyiség változtatás: TID'.$tac['id'].' => '.$tac['val'].' - MENTVE', $post[allapotID][$orderID] );
		}
		...
		foreach($termekArChange as $tac){
			$this->logOrderChange( $orderID, __LINE__, 'Termék árváltozás változtatás: TID'.$tac['id'].' => '.$tac['val'], $post[allapotID][$orderID] );
			$changedData[termekAr]	= 1;
			$this->db->update(
				'order_termekek',
				array(
			...
			$this->logOrderChange( $orderID, __LINE__, 'Termék árváltozás változtatás: TID'.$tac['id'].' => '.$tac['val'].' - MENTVE', $post[allapotID][$orderID] );
		}
		...
		$this->logOrderChange( $orderID, __LINE__, 'Számlázási adat mentése - START', $post[allapotID][$orderID] );
		$szamlazasi_keys = addslashes(json_encode($szamlazasi_adat,JSON_UNESCAPED_UNICODE));
		$this->db->update(
			'orders',
			array(
		...
		$this->logOrderChange( $orderID, __LINE__, 'Számlázási adat mentése - MENTVE', $post[allapotID][$orderID] );
		...
		$this->logOrderChange( $orderID, __LINE__, 'Szállítási adat mentése - START', $post[allapotID][$orderID] );
		$szallitasi_keys = addslashes(json_encode($szallitasi_adat,JSON_UNESCAPED_UNICODE));
		$this->db->update(
			'orders',
			array(
		...
		$this->logOrderChange( $orderID, __LINE__, 'Szállítási adat mentése - MENTVE', $post[allapotID][$orderID] );

		// E-mail Értesítő kiküldése
		// User alert
		$this->logOrderChange( $orderID, __LINE__, 'Megrendelés adatok lekérése: getOrderData() - START', $post[allapotID][$orderID] );
		$orderData = $this->getOrderData($accessKey);
		$this->logOrderChange( $orderID, __LINE__, 'Megrendelés adatok lekérése: getOrderData() - END', $post[allapotID][$orderID] );
		extract($orderData);
		...
		/* */
		$this->logOrderChange( $orderID, __LINE__, 'CLORADE - START', $post[allapotID][$orderID] );
	        ...
	        // NAV
	        $buyer["iranyitoszam"] 		= $szam_data['irsz'];
	        $buyer["telepules"] 		= $szam_data['city'];

	        if(!empty($szam_data['kerulet']))
	        $buyer["kerulet"] 			= $szam_data['kerulet'];

	        $buyer["kozterulet_neve"] 	= $szam_data['uhsz'];

	        if(!empty($szam_data['kozterulet_jellege']))
	        $buyer["kozterulet_jellege"]= $szam_data['kozterulet_jellege'];

	    	if(!empty($szam_data['hazszam']))
	        $buyer["hazszam"] 			= $szam_data['hazszam'];

	    	if(!empty($szam_data['epulet']))
	        $buyer["epulet"] 			= $szam_data['epulet'];

	    	if(!empty($szam_data['lepcsohaz']))
	        $buyer["lepcsohaz"] 		= $szam_data['lepcsohaz'];

	    	if(!empty($szam_data['szint']))
	        $buyer["szint"] 			= $szam_data['szint'];

	    	if(!empty($szam_data['ajto']))
	        $buyer["ajto"] 				= $szam_data['ajto'];
	        ...
	        if(!empty($szall_data['kozterulet_jellege']))
	        {
				$comment .= 'Cím: '.$szall_data['irsz'].' '.$szall_data['uhsz']. ' '.$szall_data['kozterulet_jellege']. ', '.$szall_data['hazszam'];

				if(!empty($szall_data['epulet']))
				$comment .= " Épület: ".$szall_data['epulet'];

				if(!empty($szall_data['lepcsohaz']))
				$comment .= " Lépcsőház: ".$szall_data['lepcsohaz'];

				if(!empty($szall_data['szint']))
				$comment .= " Szint: ".$szall_data['szint'];

				if(!empty($szall_data['ajto']))
				$comment .= " Ajtó: ".$szall_data['ajto'];
			...
			/* */
			// Ez küldi ki a Clorade felé a vásárlás adatokat Teljesített állapot esetén, most kikommentáltam, hogy így át lehet-e állatíni.
			// Most vissza
			if( defined('DO_WEBSHOPSALE') && DO_WEBSHOPSALE )
			{
				...
				$this->logOrderChange( $orderID, __LINE__, 'CLORADE - REQUEST - END', $post[allapotID][$orderID] );
		...
		// Forgalom bejegyzése @ Teljesített megrendelés
		if( $updateData['allapot'] == $this->settings['flagkey_orderstatus_done'] ){
			...
			$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - START', $post[allapotID][$orderID] );

			// Törzsvásárlói kedvezmény értesítés
			if( $user && !empty($user['email']) )
			{
				$total_ar = (float) 0;
				$total_items_ar_brutto = 0;

				foreach ( $orderData['items'] as $cartItems ) {
					$total_ar += ($cartItems['ar'] * $cartItems['me']);
				}

				$total_items_ar_brutto = $total_ar;

				$check_torzs_value = $user['torzsvasarloi_kedvezmeny_next_price_step'] - ( $total_items_ar_brutto + $totalOrderPrice );

				$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - $check_torzs_alerted', $post[allapotID][$orderID] );

				$check_torzs_alerted = $this->db->query("SELECT 1 FROM email_ertesitesek WHERE mode = 'torzsvasarloi_kedvezmeny' and email = '$user[email]';")->rowCount();

				$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - $check_torzs_alerted - QUERY OK', $post[allapotID][$orderID] );

				if( $check_torzs_value < 0 && $check_torzs_alerted === 0 ) {

					$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - INSERT alert email: email_ertesitesek - START', $post[allapotID][$orderID] );
					$this->db->insert(
						'email_ertesitesek',
						array(
							'email' => $user['email'],
							'mode' 	=> 'torzsvasarloi_kedvezmeny',
							'ertek' => $user['torzsvasarloi_kedvezmeny_next_price_step']
						)
					);
					$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - INSERT alert email: email_ertesitesek - END', $post[allapotID][$orderID] );

					$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - EMAIL - START', $post[allapotID][$orderID] );
					...
					$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - EMAIL - PREPARED', $post[allapotID][$orderID] );
					...
					$arg = array(
						...
						'nev' 			=> $nev,
						'kedvezmeny' 	=> $user['torzsvasarloi_kedvezmeny']
					);

					$mail->setSubject( 'Törzsvásárlói kedvezmény: Aktiválva!' );
					$mail->setMsg( (new Template( VIEW . 'templates/mail/' ))->get( 'user_order_torzsvasarloi_kedvezmeny_ertesites', $arg ) );
					$re = $mail->sendMail();
					$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - EMAIL - SENDED', $post[allapotID][$orderID] );
				}

				$this->logOrderChange( $orderID, __LINE__, 'Törzsvásárlói kedvezmény - END', $post[allapotID][$orderID] );
			}
		}

		$this->logOrderChange( $orderID, __LINE__, 'EMAIL ALERT - START', $post[allapotID][$orderID] );
		if( isset( $post['alert_email_out'][$orderID] ) ) {
			$this->logOrderChange( $orderID, __LINE__, 'EMAIL ALERT - ALLOW', $post[allapotID][$orderID] );
			$this->logOrderChange( $orderID, __LINE__, 'EMAIL ALERT - PREPARE', $post[allapotID][$orderID] );
			$mail = new Mailer(
				$this->settings['page_title'],
				$this->settings['email_noreply_address'],
				$this->settings['mail_sender_mode']
			...
			$arg = array(
				...
				'kedvezmeny' 	=> $orderData['kedvezmeny_szazalek'],
				'szamlazasi_keys' => $szamlazasi_keys,
				'szallitasi_keys' => $szallitasi_keys,
			...
			$re = $mail->sendMail();
			$this->logOrderChange( $orderID, __LINE__, 'EMAIL ALERT - SENDED', $post[allapotID][$orderID] );
		} else {
			$this->logOrderChange( $orderID, __LINE__, 'EMAIL ALERT - DISALLOW', $post[allapotID][$orderID] );
		}



		$this->logorderstep = 999999;
		$this->logOrderChange( $orderID, __LINE__, 'END', $post[allapotID][$orderID] );
	...
	public function getAllUploadedProductImages( $arg = array() )
	{
		...
		if( count($log_img) > 0 ) {
			$deb = $this->db->multi_insert(
				'shop_termek_kepek_valtozasok',
				array( 'cim', 'eleresi_ut', 'utoljara_modositva' ),
				$log_img,
				array( 'duplicate_keys' => array( 'eleresi_ut', 'utoljara_modositva' ) )
			);
	...
	public function replyToMessage( $msgData, $post)
	{
		...
		// Válasz a feladónak
		$mail = new Mailer( $this->settings['page_title'], $this->settings['reply_email'], $this->settings['mail_sender_mode'] );
